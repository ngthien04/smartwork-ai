...
export default function CalendarPage() {
  ...
  const [viewingTaskEvent, setViewingTaskEvent] = useState<UnifiedEvent | null>(null);
  ...
  const taskDeadlinesQuery = useQuery({
    queryKey: ['taskDeadlines', start.format('YYYY-MM-DD'), end.format('YYYY-MM-DD')],
    queryFn: async () => {
      const res = await taskCalendarServices.getTaskDeadlines(start.toISOString(), end.toISOString());
      return res.data || [];
    },
    staleTime: 0,
    refetchOnWindowFocus: true,
  ...
  const unifiedEvents: UnifiedEvent[] = useMemo(() => {
    ...
    const tasks = (taskDeadlinesQuery.data || []).map((t) => {
      ...
      const startTime = due.hour() === 0 && due.minute() === 0 ? due.hour(9).minute(0) : due;
            ...
            <Tooltip key={id} title={title}>
              <div
                ...
                style={{
                  ...
                  padding: '2px 6px',
                  borderRadius: 10,
                  background: ev.source === 'task' ? 'rgba(255,77,79,0.08)' : '#f5f5f5',
                  border: ev.source === 'task' ? '1px solid rgba(255,77,79,0.18)' : '1px solid rgba(0,0,0,0.06)',
                  marginBottom: 4,
                }}
              >
                <span
                        ...
                        isTask
                          ? [
                            <Button
                              ...
                              size="small"
                              type="primary"
                              onClick={() => navigate(`/tasks/${(ev as any).taskId}`)}
                            >
                              Mở task
                            </Button>,
                          ]
                          : [
                            <Button key="edit" size="small" onClick={() => openEditManualModal(ev as any)}>
                              Sửa
                            </Button>,
                            <Popconfirm
                              key="del"
                              ...
                              onConfirm={() => deleteEventMutation.mutate((ev as any)._id)}
                            >
                      ...
                      <List.Item.Meta
                        title={
                          <Space wrap>
                            ...
                            {isTask ? <Tag color={priorityColor((ev as any).priority)}>deadline</Tag> : <Tag>event</Tag>}
                          </Space>
                        }
                        description={
                          <Space direction="vertical" size={2}>
                            ...
                            {!isTask && (ev as any).location ? (
                              <Space>
                                <EnvironmentOutlined />
                                <Text type="secondary">{(ev as any).location}</Text>
                              </Space>
                            ) : null}
        ...
        {viewingTaskEvent && viewingTaskEvent.source === 'task' ? (
          <Space direction="vertical" className="w-full">
            <div>
              ...
              <div style={{ marginTop: 6 }}>
                <Tag color={priorityColor(viewingTaskEvent.priority)}>{viewingTaskEvent.priority || 'normal'}</Tag>
                {viewingTaskEvent.status ? <Tag>{viewingTaskEvent.status}</Tag> : null}
              </div>
            </div>
